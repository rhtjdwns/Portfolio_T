...
## 경력
1. 2025-05-22 ~ 2025-08-20 스마일게이트 주최 샌드박스 게임 제작툴 검증 프로젝트 진행 및 스마일게이트 인턴쉽 진행
- 자사 제작툴을 사용하여 게임을 만들어 툴에 대한 버그, 개선점을 검토하여 제시, 우수 참여자로 선정되어 이후 인턴쉽(2025-07-20 ~ 2025-08-20)을 진행하였으며 인턴쉽 진행기간 동안 제작툴을 사용하여 게임을 제작. 다른 게임 엔진에 비해 버그가 많고 부족한 점이 많은 제작툴임에도 불구하고 내부에서 "기대했던 것보다 더 퀄리티가 높은 게임"이라는 평가의 게임을 제작하였음.
- 해당 검증을 통해 제작툴의 방향성을 기존과 다르게 잡는 영향을 주었음.
...
- 플레이어의 이동속도에 따라 발걸음 속도를 조절하거나, 각 장소마다 사운드를 따로 적용하기 위하여 기존 유니티의 사운드 시스템보다 FMOD를 사용하여 더 정교한 사운드 시스템을 구현하였음
- 적의 종류가 많은 만큼 하나의 부모로 공통된 점을 작성하고 자식 클래스로 각 적 오브젝트만의 특별한 로직(EX. 적의 종류에 따라 달라지는 공격)을 작성함.

### 프로젝트 후기
- 혼자서 많은 그래픽 인원과 기획을 맡아야하는 만큼 명확하게 할 수 있는 일과 기간 내에 힘들 것 같은 일을 분리하여 기획, 그래픽 팀원과 소통하였다. 하루 10~12시간 이상 작업시간을 투자하며 로직과 관련된 부분을 빠르게 끝내고 그래픽에서 오는 리소스를 적용 및 교체하는 방식으로 진행하였다. 많은 일을 담당해야하는 만큼 레벨 디자인, 애니메이션 수정 같은 일은 애니메이터, 기획 쪽에서 쉽게 처리할 수 있도록 따로 프로퍼티를 만들어 인스펙터로 빼어 이를 에디터 상에서 조절할 수 있도록 하였다.
- 프로그래머가 1명인데에 비해서 그래픽 인원이 많기 때문에 로직과 디버깅 관련된 부분에선 빠르게 끝내기 위해 하루 작업시간을 많이 가져갈 수 밖에 없었고 해당 작업기간 동안 프로젝트를 완성하리라는 책임감을 가지고 끝까지 이행하였다.
...
- 해당 클래스를 상속받아 여러 개의 스킬을 보스에게서 구현하기 위함.
...
- 오류가 발생했을 때 어떤 상태에서 문제가 있는 지 용이하게 확인하기 위해, 새로운 상태를 추가하여 패턴을 다양화하기 용이하기 때문에 FSM 패턴을 사용하였다.
...
- 포물선 운동은 유니티의 RigidBody가 중력, 작용하는 힘의 방향 등 계산을 자동적으로 해주어 이것을 이용하는 게 좋다고 판단함.
...
- 중간 보스와 같은 형태의 FSM이며 플레이어의 행동에 따라서 각각의 상태를 가질 필요가 있었기 때문에 같은 패턴을 사용하였다.
...
- Stack을 사용하기 보다 커맨드를 순차적으로 검사해야하고 플레이어가 누르는 방향에 따라 스킬이 나가는 형식도 변화하기 때문에 검색이 용이한 List를 쓰는 것이 낫다고 판단하였다.
...
- 직접 움직이는 플레이어와 달리, 상대하게 되는 AI는 직접 움직일 수 없으므로 공격, 이동, 스킬 3가지로 나누고 공격 가능한 시간, 이동 가능한 시간, 스킬 사용 시간을 정하여 FSM 패턴을 조성함.
- 스킬을 스토리지에 담아 플레이어와 AI가 해당 스킬 쿨타임이 돌았을 경우 사용 가능하도록 설계함.

![2025-10-23+17-47-55+(1)](https://github.com/user-attachments/assets/9e9102ca-c35d-4311-81ae-ef9c5b8b54a8)
